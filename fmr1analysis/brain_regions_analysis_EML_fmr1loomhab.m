%%% this code is based on the one I used to analyse my habituation
%%% experiments. It was not working because there was a problem with the
%%% indexing of the idx_Plane and Numbers (which is solved now in the
%%% quick1stanalysis_fmr1loomhab.m script) and because there was a problem
%%% with the sorting... not sure why. so I tagged the ROIs with the ROIs_for_ANTS_Emm2_fmr1loomhab_tagged
%%% script and then I could use their proper indexing. This is done in the 
%%% brain_regions_analysis_EML_fmr1loomhab_solvingproblems.m script
...
%%% it seems that I am getting the same problem than before where 1 ROIs is missplaced from fish.
%%% but i think I can still keep going. because i have the same total
%%% number of ROIs
...
for i=1:length(CSV_Files)
    ...
    truth(i)=size(temp_warp,1)==sum(idx_Fish==str2num(Fishname)) %%% important!! i need to have an idx_Fish with all the fish.
end
clearvars i temp CSV_Files Fishname
...
figure;scatter(ROI_temp(idx_rsq,1),ROI_temp(idx_rsq,2),'filled');
...
%%
%save('BrainReg.mat','PerBrainRegions','RegionList','ROI_temp2','-v7.3');
