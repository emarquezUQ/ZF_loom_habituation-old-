...
%%%% the CL4 and per cluster. but for the hind brain. to try to find the
%%%% cluster of cells that change from nonhab to slopehab depending on the
%%%% ISI

%%%% so a first look at the results suggest that the masks of the hindbrain
%%%% were i can find the changes due to ISI are Rhombomre1, Cerebellum,
%%%% Corpus Cerebelli. And maybe a bit in  Gad1bCluster1 and Gad1bCluster2.
%%% so this pretty much suggest that these are neurons of the medial deep
%%% part of the cerebellum. 


load('All_More_BrainReg3.mat')
...
for i=1:length(RegionList)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_f20.(strcat('reg',num2str(i))).idx,temp_idx))/sum(ismember(temp_idx,intersect(idx_rsq_test_f20short_cleaned,temp_idx_fish))); %%length(temp_idx)
end
...
save('All_Prop_CL4_per_fish_rsq_perCluster2_extraHindbrain.mat','All_Prop_CL4_per_fish');
...
boxplot(All_Prop_CL4_per_fish.(datasets(data,:)).(clustersF{clust}),'Colors',colors{data}); xtickangle(45),title((clustersF{clust,1})); ylim([0 0.2]); %set(gca,'xticklabel',RegionList)
hold on;
end
