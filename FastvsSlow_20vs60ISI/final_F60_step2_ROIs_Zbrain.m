...
%%%% this is for testing with only f60 fish
...
for fish_nb=1:length(Fish_list_f60)
    %temp_warp=num2str(Fish_list(fish_nb)); %Gilles
     ...
     %  i added this cause fish 3 and 4 are missing for f60... but it
     %  doesnt seem to be working... i think is because the goodnumbers has
     %  already been added from before. but maybe i can delete them
     %  afterwards... but it would mess up al the indexing and I would need
     %  to fix that too...
    if ismember(str2num(Fishname),str2num(Fish_list_f60))
...
%  gooodmaps_s20=load('final_S20_step1.mat','gooodmaps');
%  gooodmaps_s60=load('final_S60_step1.mat','gooodmaps');
%  gooodmaps_s20=gooodmaps_s20.gooodmaps;
%  gooodmaps_s60=gooodmaps_s60.gooodmaps;
...
for i=1:length(unique(High_corr_Nb_f60_3))
    ...
    filename=strcat('__Coords_clust_f60_CL7_',num2str(i),'.csv');
...
for i=gooodmaps
    ...
    filename=strcat('__Coords_clust_f60_CL4_',num2str(i),'.csv');
...
figure;imagesc(ZS_f60(intersect(PerBrainRegions.Telencephalon.idx,idx_rsq_test_f60short3),ZS_short_F60), [-0.5 4]);colormap hot

figure;imagesc(ZS_f60(intersect(PerBrainRegions.Thalamus.idx,idx_rsq_test_f60short3),ZS_short_F60), [-0.5 4]);colormap hot

figure;imagesc(ZS_f60(intersect(PerBrainRegions.Pretectum.idx,idx_rsq_test_f60short3),ZS_short_F60), [-0.5 4]);colormap hot

figure;imagesc(ZS_f60(intersect(PerBrainRegions.Tectum.idx,idx_rsq_test_f60short3),ZS_short_F60), [-0.5 4]);colormap hot
