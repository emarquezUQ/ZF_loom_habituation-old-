...
%%%% the CL4 and per cluster. this time I am just checking if I am missing
%%%% an interesting area



%%% from the nonhab neurons, it seems that there are more in the right
%%% habenula than in the left habenula. there is also some details of the
%%% subtypes of precetal and pallial neurons that respond but is not very clear.


%%% inhibited neurons seems to be mostly in the ventral thalamus


load('All_More_BrainReg4.mat')
...
for j=1:size(clustersF,1)
   figure; 
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_f20.(strcat('reg',num2str(i))).idx,idx_temp))/sum(ismember(idx_temp,idx_rsq_test_f20short_cleaned));
    end
bar(ratioperRegion);set(gca,'xticklabel',RegionList);% xtickangle(45),title((clustersF{j,1})); ylim([0 1])
...
for i=1:length(RegionList)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_f20.(strcat('reg',num2str(i))).idx,temp_idx))/sum(ismember(temp_idx,intersect(idx_rsq_test_f20short_cleaned,temp_idx_fish))); %%length(temp_idx)  %% the formula as it is is redundant cause i get the same value with temp_idx and the divider formula
end
...
% for i=1:length(fish)
%    
%     plot(Prop_CL4_per_fish_f20{clust}(i,:),'.');set(gca,'xticklabel',RegionList),xtickangle(45),title((clustersF{clust,1})); ylim([0 1])
%      
%     hold on;
...
%subplot(3,3,counter);
boxplot(Prop_CL4_per_fish_f20{clust});%set(gca,'xticklabel',RegionList),xtickangle(45),title((clustersF{clust,1})); ylim([0 1])
 counter=counter+1;
...
for clust=1:size(clustersF,1)
    figure;
%subplot(3,3,counter);
boxplot(All_Prop_CL4_per_fish.f20.(clustersF{clust}));set(gca,'xticklabel',RegionList),xtickangle(45),title((clustersF{clust,1})); ylim([0 1])
 counter=counter+1;
end
...
for j=1:size(clustersF,1)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_f60.(strcat('reg',num2str(i))).idx,idx_temp))/sum(ismember(idx_temp,idx_rsq_test_f60short_cleaned)); 
end
...
for i=1:length(RegionList)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_f60.(strcat('reg',num2str(i))).idx,temp_idx))/sum(ismember(temp_idx,intersect(idx_rsq_test_f60short_cleaned,temp_idx_fish))); %%length(temp_idx)
end
...
for j=1:size(clustersS,1)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_s20.(strcat('reg',num2str(i))).idx,idx_temp))/sum(ismember(idx_temp,idx_rsq_test_s20short_cleaned));
end
...
for i=1:length(RegionList)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_s20.(strcat('reg',num2str(i))).idx,temp_idx))/sum(ismember(temp_idx,intersect(idx_rsq_test_s20short_cleaned,temp_idx_fish))); %% length(temp_idx)
end
...
for j=1:size(clustersS,1)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_s60.(strcat('reg',num2str(i))).idx,idx_temp))/sum(ismember(idx_temp,idx_rsq_test_s60short_cleaned));
end
subplot(3,3,counter);bar(ratioperRegion);set(gca,'xticklabel',RegionList),xtickangle(45),title((clustersS{j,1})); ylim([0 1])
...
for i=1:length(RegionList)
    ...
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions_s60.(strcat('reg',num2str(i))).idx,temp_idx))/sum(ismember(temp_idx,intersect(idx_rsq_test_s60short_cleaned,temp_idx_fish))); %%length(temp_idx)
end
...
save('All_Prop_CL4_per_fish_rsq_perCluster2_extraRestOfBrain.mat','All_Prop_CL4_per_fish');
...
boxplot(All_Prop_CL4_per_fish.(datasets(data,:)).(clustersF{clust}),'Colors',colors{data}); xtickangle(90),title((clustersF{clust,1})); ylim([0 1]); set(gca,'xticklabel',RegionList)
hold on;
end
