...
for i=1:length(unique(High_corr_Nb_f20))
    ...
    filename=strcat('__Coords_clust_f20_CL7_',num2str(i),'.csv');
...
%%% to check 
%figure;plot(mean(ZS_f20(idx_temp2,:)));
...
for i=gooodmaps
    ...
    filename=strcat('__Coords_clust_f20_CL4_',num2str(i),'.csv');
...
figure;imagesc(ZS_f20(intersect(PerBrainRegions.Telencephalon.idx,idx_rsq_test_f20short),:), [-0.5 4]);colormap hot

figure;imagesc(ZS_f20(intersect(PerBrainRegions.Thalamus.idx,idx_rsq_test_f20short),:), [-0.5 4]);colormap hot

figure;imagesc(ZS_f20(intersect(PerBrainRegions.Pretectum.idx,idx_rsq_test_f20short),:), [-0.5 4]);colormap hot

figure;imagesc(ZS_f20(intersect(PerBrainRegions.Tectum.idx,idx_rsq_test_f20short),:), [-0.5 4]);colormap hot
...
figure;plot(mean(ZS_f20(intersect(PerBrainRegions.Telencephalon.idx,idx_rsq_test_f20short),:)));

%figure;
hold on;plot(mean(ZS_f20(intersect(PerBrainRegions.Tectum.idx,idx_rsq_test_f20short),:)));


%%% this is to check if the numbers of the slope-hab cluster make sense
idx_temp=find(High_corr_Nb_f20_short==6);

for i=1:length(RegionList2)
    regionName=RegionList2{i};
    amountperRegion(:,i)=sum(ismember(PerBrainRegions.(regionName).idx,idx_rsq_test_f20short(idx_temp)));
end
...
for i=1:length(RegionList2)
    regionName=RegionList2{i};
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions.(regionName).idx,idx_rsq_test_f20short(idx_temp)))/sum(ismember(PerBrainRegions.(regionName).idx,idx_rsq_test_f20short));
end
...
idx_temp2=find(High_corr_Nb_f20_short==2);
counter=1;
figure;
for j=[2 4 5]
    ...
    for i=1:length(RegionList2)
    regionName=RegionList2{i};
    ratioperRegion(:,i)=sum(ismember(PerBrainRegions.(regionName).idx,idx_rsq_test_f20short(idx_temp)))/sum(ismember(PerBrainRegions.(regionName).idx,idx_rsq_test_f20short(idx_temp2)));
end
subplot(2,2,counter);bar(ratioperRegion);set(gca,'xticklabel',RegionList2);
...
%print(gcf,'ratios_perReg_fasthab_CL7_f20','-dpdf','-bestfit');
...
subplot(1,3,counter);bar(ratioperRegion);set(gca,'xticklabel',RegionList2); ylim([0 1])
...
for j=2
    ...
    %subplot(3,3,counter);
    plot(mean(ZS_f20(intersect(PerBrainRegions.(regionName).idx,idx_rsq_test_f20short(idx_temp)),55:100)));
   hold on; 
   counter=counter+1;
...
%print(gcf,'mean1st_perReg_fasthab_CL4_f20','-dpdf','-bestfit');

%%% to look at the Msystem responses... not much...
figure;imagesc(ZS_f20(intersect(PerBrainRegions.Msystem.idx,idx_rsq_test_f20short),:), [-05 5]); colormap hot
