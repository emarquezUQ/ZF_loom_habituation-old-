...
load('final_S60_step1.mat','MatFiles_s60','ZS_s60','idx_Fish_s60','idx_Plane_s60','rawregressS60','idx_rsq_test_s60short','High_corr_Nb_s60','High_corr_Nb_s60_short','ZS_short_S60');
...
%%%%first 3 looms. for s60 it would be from 180 onwards

%%% something weird is happening... i had to change some things to avoid
%%% problems with indexing but now I get very little results. i need to
%%% check if this is normal. although with s20 it seemed to work.
...
%for fish=Fish_with_behav_s60
  % if ismember(fish,unique(idx_Fish_s60))  
    ...
    %temp_ZS=ZS_s60(find(idx_Fish_s60==fish),:);ModelResults_temp=[];
        ...
        %%% before and after.
        %mdl=fitlm(squeeze(squeeze(Movements(find(Fish_with_behav_s60==fish),find(Movements2(find(Fish_with_behav_s60==fish),:)))))',temp_ZS(i,find(Movements2(find(Fish_with_behav_s60==fish),:))));
...
%%% 0.2 is a good threshold in general but for s60 is giving me bad
%%% results. Many fish have soo many movements that not the regressors are
%%% not very effective.
%%% there is only a few fish i get their motor responses and
%%% allmost all ROIs belong to fish 16 only. i can increase the ROIs
%%% lowering the threshold but 0.1 is probalby too low as I start getting
%%% weird ROIs. myabe in this case 0.15 is a better option. Or I maybe need
%%% to use the 'motor seeds' as in the Misha biorxiv paper.
...
for fish=1:length(Fish_with_behav_s60)
   if ismember(Fish_with_behav_s60(fish),unique(idx_Fish_s60))  
    temp_idx=find(idx_Fish_s60==Fish_with_behav_s60(fish)); idx_Mov_temp=[];   
    rsq_temp=[ModelResults_allmovPerFish{1,counter}.rsquared];
    idx_Mov_temp=temp_idx(find(rsq_temp>0.15 & rsq_temp<1),:);
    idx_rsq_Mov3{counter}=idx_Mov_temp;
    rsquare_loom_allmov2{counter}=rsq_temp;
    counter=counter+1;
   else
   end    
...
idx_rsq_Mov3=vertcat(idx_rsq_Mov3{:});
figure; imagesc(ZS_s60(idx_rsq_Mov3,ZS_short_S60), [-0.5 4]);colormap hot
...
save('Tail_mov_S60_2.mat','idx_rsq_Mov3','-v7.3');
...
%%% To get the ROIs of of the movements 
    ...
    CSV_temp=ROI_temp2(idx_rsq_Mov3,:);

    CSV_temp(:,3)=CSV_temp(:,3); %%% why?

    CSV_temp(:,4)=1;

    filename=strcat('__Coords_Tail_mov_s60_','.csv');
...
figure;
scatter3(ROI_temp2(idx_rsq_test_s60short,1),ROI_temp2(idx_rsq_test_s60short,2),ROI_temp2(idx_rsq_test_s60short,3)); %%% this is my rotation
hold on;
scatter3(ROI_temp2(idx_rsq_Mov,1),ROI_temp2(idx_rsq_Mov,2),ROI_temp2(idx_rsq_Mov,3)); %%% this is my rotation


figure;
scatter(ROI_temp2(idx_rsq_test_s60short,1),ROI_temp2(idx_rsq_test_s60short,2),'.'); %%% this is my rotation
hold on;
scatter(ROI_temp2(idx_rsq_Mov3,1),ROI_temp2(idx_rsq_Mov3,2),'.'); %%% this is my rotation


figure;
scatter(ROI_temp2(intersect(find(idx_Fish_s60==1),idx_rsq_test_s60short),1),ROI_temp2(intersect(find(idx_Fish_s60==1),idx_rsq_test_s60short),2),'.'); %%% this is my rotation
hold on;
scatter(ROI_temp2(intersect(find(idx_Fish_s60==1),idx_rsq_Mov),1),ROI_temp2(intersect(find(idx_Fish_s60==1),idx_rsq_Mov),2),'.'); %%% this is my rotation


idx_temp1=idx_rsq_test_s60short(find(High_corr_Nb_s60_short==3));
idx_temp2=idx_rsq_test_s60short(find(High_corr_Nb_s60_short==1));

figure;
scatter3(ROI_temp2(intersect(PerBrainRegions.Pallium.idx,idx_rsq_test_s60short),1),ROI_temp2(intersect(PerBrainRegions.Pallium.idx,idx_rsq_test_s60short),2),ROI_temp2(intersect(PerBrainRegions.Pallium.idx,idx_rsq_test_s60short),3),'.'); %%% this is my rotation
hold on;
...
hold on;
scatter3(ROI_temp2(intersect(PerBrainRegions.Subpallium.idx,idx_rsq_test_s60short),1),ROI_temp2(intersect(PerBrainRegions.Subpallium.idx,idx_rsq_test_s60short),2),ROI_temp2(intersect(PerBrainRegions.Subpallium.idx,idx_rsq_test_s60short),3),'.'); %%% this is my rotation
hold on;
...
figure; 
%scatter3(ROI_temp2(intersect(PerBrainRegions.Telencephalon.idx,idx_rsq_test_s60short),1),ROI_temp2(intersect(PerBrainRegions.Telencephalon.idx,idx_rsq_test_s60short),2),ROI_temp2(intersect(PerBrainRegions.Telencephalon.idx,idx_rsq_test_s60short),3)); %%% this is my rotation
hold on;
     for fish=1:length(Fish_with_behav_s60)
        if ismember(Fish_with_behav_s60(fish),unique(idx_Fish_s60)) %%isempty(Correlation_movement_post3{fish})~=1 % idx_Fish_s60==Fish_with_behav_s60(fish)
        idx_temp_fish=intersect(find(idx_Fish_s60==Fish_with_behav_s60(fish)),idx_rsq_test_s60short);
        idx_temp_nonhab=idx_rsq_test_s60short(find(High_corr_Nb_s60_short==3));
...
figure;imagesc(ZS_s60(intersect(idx_temp1,find(idx_Fish_s60==1)),ZS_short_S60));

figure;plot(mean(ZS_s60(intersect(idx_temp1,find(idx_Fish_s60==1)),ZS_short_S60)));
...
figure;imagesc(ZS_s60(intersect(idx_temp2,find(idx_Fish_s60==1)),ZS_short_S60));
figure;imagesc(ZS_s60(intersect(idx_temp3,find(idx_Fish_s60==1)),ZS_short_S60));

figure;plot(mean(ZS_s60(intersect(idx_temp2,find(idx_Fish_s60==1)),ZS_short_S60)));
figure;plot(mean(ZS_s60(intersect(idx_temp3,find(idx_Fish_s60==1)),ZS_short_S60)));

figure;
scatter3(ROI_temp2(intersect(PerBrainRegions.Pallium.idx,idx_rsq_test_s60short),1),ROI_temp2(intersect(PerBrainRegions.Pallium.idx,idx_rsq_test_s60short),2),ROI_temp2(intersect(PerBrainRegions.Pallium.idx,idx_rsq_test_s60short),3),'.'); %%% this is my rotation
hold on;
scatter3(ROI_temp2(intersect(idx_temp2,find(idx_Fish_s60==1)),1),ROI_temp2(intersect(idx_temp2,find(idx_Fish_s60==1)),2),ROI_temp2(intersect(idx_temp2,find(idx_Fish_s60==1)),3),'.'); %%% this is my rotation
hold on;
scatter3(ROI_temp2(intersect(idx_temp3,find(idx_Fish_s60==1)),1),ROI_temp2(intersect(idx_temp3,find(idx_Fish_s60==1)),2),ROI_temp2(intersect(idx_temp3,find(idx_Fish_s60==1)),3),'.'); %%% this is my rotation
