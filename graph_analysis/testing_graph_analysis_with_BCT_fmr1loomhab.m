...
subplot(1,7,counter);
plot(Zbrain_brainMask2D(:,1),Zbrain_brainMask2D(:,2),'k'); xlim([300 1400]); 
hold on; scatter(Nodes2.Mod_loc(keep,1),Nodes2.Mod_loc(keep,2),25,(MatAll_corrected.(group).(loom{i}).deg-MatAll_corrected.(groupnames{g-1,1}).(loom{i}).deg),'filled');colormap('jet');caxis([low high]);view(-90,90);%colorbar; 
...
[gamma_min,gamma_max]=gammaRange(Data_corrMatK.fmr1.Mean_corrMat{1,2});
S = fixedResSamples(Data_corrMatK.fmr1.Mean_corrMat{1,2}, 10000, 'Gamma', 1.2);
...
for k=[1:21]
    ...
    temp_mat1=Data_corrMatK.control.Mean_corrMat{1,k};
    temp_mat1(isnan(temp_mat1))=0;
    
    temp_mat2=Data_corrMatK.fmr1.Mean_corrMat{1,k};
...
for k=[1:21]
    ...
    %R=Data_corrMatK.control.Mean_corrMat{1,k};   
...
threshold = 0.2; %  minimum correlation to plot
...
hold on;
gscatter(Nodes2.Mod_loc(keep,1),Nodes2.Mod_loc(keep,2),Nodes2.Mod_clust,'grbbggrm','.',20,'off');
